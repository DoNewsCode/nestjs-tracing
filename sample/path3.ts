...
const config: TracingConfig = {
  serviceName: 'my-awesome-service3',
  sampler: {
    param: 1,
    type: 'const',
  },
...
export function path3(headers: Record<string, string>): Promise<any> {
  return new Promise((resolve) => {
    ...
    span.log({ event: 'path3_request_start', ctx });
    setTimeout(() => {
      span.log({ event: 'path3_request_end' });
      span.finish();
      resolve();
    }, 1000 * Math.random());
  });
